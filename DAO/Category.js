...
CategoryDAO.prototype.ListAll = function(callback){
	sql = 'SELECT p.category_id as category_id, p.category_name as category_name, p.category_desc as category_desc, a.sub_name as sub_name FROM category as p, sub_category as a WHERE p.sub_id = a.sub_id';
...
CategoryDAO.prototype.Create = function(array,callback){
	var sql = "INSERT INTO category (sub_id, category_name ,category_desc) VALUES ?";
	var values = [[
	 	array.sub_id, 
	 	array.category_name,  
	 	array.category_desc  
...
CategoryDAO.prototype.Read = function(id,callback){
	this._conn.query('SELECT * FROM category WHERE category_id = ?', id, callback);
}
CategoryDAO.prototype.Update = function(array,callback){	
	this._conn.query('UPDATE category SET ? WHERE category_id = ? ', [{
	  	sub_id: array.sub_id,
...
CategoryDAO.prototype.Delete = function(id,callback){
	this._conn.query('DELETE FROM category WHERE category_id = "'+ id +'"', callback);
}
